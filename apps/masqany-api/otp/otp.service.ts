...
import { OtpResponse, SendEmailResponse, ValidateOtpDto } from "./otp.interface";
...
const OtpService = {
    generateOtp: async(): Promise<OtpResponse> =>{
        ...
        const expirationDate = new Date(localeDate.setSeconds(localeDate.getSeconds() + expirationDuration));
    ...
    sendEmail: async (otp: string): Promise<SendEmailResponse> => {
        try {
            ...
            const response = await mg.messages.create('sandboxa63db9a212cc4158b5244f3b972818a0.mailgun.org', {
